...
Schauen wir uns nun den Python-Code dazu an, Listing \ref{lis:py} enthält einen entsprechenden Auszug aus der Datei, den kompletten Quellcode mit einem ausführlich dokumentierten Beispiel findet der geneigte Leser unter \url{http://www.uweziegenhagen.de/?p=3513}. Die durch \LaTeX\ erstellte \texttt{<Stichwort:Seite>} Datei wird hier über den \verb|read_csv| Befehl in einen pandas DataFrame geladen, diesen DataFrame kann man sich als Excel-Tabelle vorstellen, in einer Spalte die Stichwörter, in der anderen die jeweilige Seitenzahl, auf der das Stichwort genannt wurde. Die \texttt{\enquote{result =}} Zeile enthält recht komplexe Logik. Hier wird nach dem jeweiligen Stichwort gruppiert und ein String erstellt, der die Seitenzahlen per Komma und Leerzeichen getrennt enthält. Dabei werden Seiten, auf denen ein Stichwort mehrfach erscheint, auch nur einmal ausgegeben.  Was von diesem Befehlt zurückgegeben wird, wird dann wieder in einen DataFrame verwandelt, um leichter damit arbeiten zu können. Über eine pandas-Funktion wird dieser DataFrame dann in eine \LaTeX-Tabelle geschrieben, die dann direkt in \LaTeX\ importiert werden kann (über den selbstdefinierten \Macro{stichworttabelle} Befehl). Zum Schluss werden in der \LaTeX-Datei mittels eines regulären Ausdrucks die \Macro{ausgabe} Befehle gesucht und die in den Befehlen vorhandenen Seitenzahlen durch die (möglicherweise) aktuellere Version im DataFrame ersetzt.
...
%%%Wer es etwas einfacher haben möchte, wählt in der Definition von \Command{backref} einen automatisch einmaligen Wert, 
...
%%%\begin{lstlisting}[style=nonumber]
%%%\define@key{catalogue}{backref}{\def\KVbackref{\ausgabe{\KVlabel}{}}}
%%%\end{lstlisting}
%%%Und für alle, die sich alle Möglichkeiten offen halten wollen:
%%%\begin{lstlisting}[style=number]
%%%\define@key{catalogue}{backref}{%
%%%	\def\KVbackref{%
%%%				\ifdef{#1}%
%%%						{\ausgabe{#1}{}}
%%%						{\ifdef{\KVlabel}%
...
%%%}
%%%\end{lstlisting}
Wenn etwas in den Katalogeintrag \Command{backref} steht, dann wird dieser Eintrag übernommen,
...
Bei den Katalogeinträgen reicht es, am Ende \Command{backref} einzutragen:
...
\end{lstlisting}
Für das aufgeführt Beispiel %Beispiel-PDF Seite 3 von catalogueentry-tex_mwe.tex
wurde die \Macro{section} des Katalogteils geändert, damit die \Macro{labelcref}-Einträge das Kürzel ›Kat. A.1‹, etc. bekommen.
\begin{lstlisting}[style=number]
\section{Katalogteil}
...
sodass man diese Varianten ausprobieren kann.\footnote{Für Python-Code:
\url{https://github.com/LukasCBossert/DTK-TeX-Python/blob/master/catalogueentry-python.tex}
